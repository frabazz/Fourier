...
void Component::fillRect(SDL_Rect *rect) {
  SDL_Rect r = {rect->x + _renderArea.x, rect->y + _renderArea.y, rect->w,
                rect->h};
...
void Component::drawLine(float x1, float y1, float x2, float y2) {
  SDL_RenderDrawLineF(_renderer, _renderArea.x + x1, _renderArea.y + y1,
                      _renderArea.x + x2, _renderArea.y + y2);
}
...
void Component::drawLineAA(float x0, float y0, float x1, float y1,
                           SDL_Color color) {
  ...
  auto setBrightness = [this, &color](double c) {
    SDL_SetRenderDrawColor(this->_renderer, c * color.r, c * color.g,
                           c * color.b, color.a);
  };
  ...
  if (steep) {
    for (int x = xpxl1 + 1; x < xpxl2; ++x) {
      ...
      drawPoint(static_cast<double>(std::floor(intery)),
                static_cast<double>(x));
      setBrightness(fpart(intery));
      drawPoint(static_cast<double>(std::floor(intery)) + 1,
                static_cast<double>(x));
      intery += gradient;
    }
  } else {
    for (int x = xpxl1 + 1; x < xpxl2; ++x) {
      setBrightness(rfpart(intery));
      drawPoint(static_cast<double>(x),
                static_cast<double>(std::floor(intery)));
      setBrightness(fpart(intery));
      drawPoint(static_cast<double>(x),
                static_cast<double>(std::floor(intery)) + 1);
      intery += gradient;
    }
  }
...
void Component::drawToolTip(std::string text, SDL_Rect *area,
                            SDL_Color bgColor) {
  SDL_Surface *surface =
      TTF_RenderText_Blended(_default_font, text.c_str(), bgColor);
